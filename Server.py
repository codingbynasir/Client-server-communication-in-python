...
def Client1():
    ...
    while True:
        ...
        if data1 == "Nasir Disconnected":
            client1_ConnectionStatus = 0
            if client2_ConnectionStatus == 1:
                connection2.sendall("Nasir Disconnected")
            if client3_ConnectionStatus == 1:
                connection3.sendall("Nasir Disconnected")
            connection1.close()
            t1.stop()
            break
        else:
...
def Client2():
    ...
    while True:
        ...
        if data2 == "Tanvir Disconnected":
            client2_ConnectionStatus = 0
            if client1_ConnectionStatus == 1:
                connection1.sendall("Tanvir Disconnected")
            if client3_ConnectionStatus == 1:
                connection3.sendall("Tanvir Disconnected")
            connection2.close()
            t2.stop()
            break
        else:
...
def Client3():
    ...
    while True:
        ...
        if data3 == "Anis Disconnected":
            client3_ConnectionStatus = 0
            if client1_ConnectionStatus == 1:
                connection1.sendall("Anis Disconnected")
            if client2_ConnectionStatus == 1:
                connection2.sendall("Anis Disconnected")
            connection3.close()
            t3.stop()
            break
        else:
    ...
    while True:
        ...
        if client1_ConnectionStatus == 1:
            if client2_arrived == 1:
                connection1.sendall("Tanvir is Online")
            if client3_arrived == 1:
                connection1.sendall("Anis is Online")

        #Tanvir is Online
        if client2_ConnectionStatus == 1:
            if client1_arrived == 1:
                connection2.sendall("Nasir is Online")
            if client3_arrived == 1:
                connection2.sendall("Anis is Online")
        
        #Anis is Online
        if client3_ConnectionStatus == 1:
            if client1_arrived == 1:
                connection3.sendall("Nasir is Online")
            if client2_arrived == 1:
                connection3.sendall("Tanvir is Online")
